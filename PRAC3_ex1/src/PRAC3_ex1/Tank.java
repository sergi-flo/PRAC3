...
public class Tank {
	 ...
	 * @throws Exception When name is longer than 40 characters, length less than 0.1, width less than 0.1, height less than 0.1 or ph is not between 0 and 14.
	 */
	public Tank(String name, String description, double length, double height, double width, String imageBackground, double temperature, int ph) throws TankException {
		setName(name);
	...
	public void setName(String name) throws TankException {
		if(name.length()>40) {
			throw new TankException(TankException.MSG_ERR_NAME_LONGER);
		}
		this.name = name;
	}
	 ...
	 * @throws Exception When the item which must be added to the tank is already in the tank or the item is null.
	 */	
	public void addItem(Item item) throws TankException{
		if(item == null) {
			throw new TankException(TankException.MSG_ERR_ITEM_NOT_OBJECT);
		}
		
		if(getItems().contains(item)) {
			throw new TankException(TankException.MSG_ERR_ITEM_IN_TANK);
		}
	...
	public void removeItem(Item item) throws TankException{
		if(!items.remove(item)){
			throw new TankException(TankException.MSG_ERR_ITEM_NOT_EXIST);		
		}
		item.setTank(null);
	}
	...
	public void removeDeadItems() {		
		for(int i = 0; i < getNumberItems(); i++) {
			if(getItem(i).getStatus() == ItemStatus.DEAD) {
				removeItem(i);
			}
		}			
	}
